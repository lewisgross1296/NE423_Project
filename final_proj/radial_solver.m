function [T, r] = radial_solver(Q,LHR,TCO,r_f,r_g,r_c,k_f,k_g,k_c,Nf,Ng,Nc)
    ...
    % Q is a vector with the improved fuel profile fission rate
    % all others are the same as before: TCO,r_f,r_g,r_c,k_f,k_g,k_c,Nf,Ng,Nc TODO
    
    hf = r_f/(Nf-1) ;
    hg = (r_g-r_f)/(Ng-1) ;
    hc = (r_c-r_g)/(Nc-1) ;
    N = Nf+Ng+Nc-2 ;
    %LHR = Q*pi*r_f^2; TO DELETE
    %radial mesh
